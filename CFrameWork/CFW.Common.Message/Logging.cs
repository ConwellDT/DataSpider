...
namespace CFW.Common
{
	...
	public class Logging
	{
		...
		public string LogFilePath
		{
			...
			{
				string strReturn = "";
				if( m_FilePath != null & m_FilePath.Length > 0)     strReturn = m_FilePath;
				else                                                strReturn = CFW.Configuration.ConfigManager.Default.ReadConfig("LOG", "LOG_PATH");

				if( strReturn.EndsWith(@"\") == false)              strReturn += @"\";

				m_FilePath = strReturn;
				return m_FilePath;
			}

            set { m_FilePath = (string)value; }
		}
		...
		public int	LogSaveDays		
		{
			get
			{
				...
				if( strValue != null && strValue.Length > 0)    m_LogSaveDays = int.Parse(strValue);
		...
		public string UserID
		{
			get
			{
				if(m_sUserID == null || m_sUserID.Length == 0)  m_sUserID = System.Net.Dns.GetHostName();
				return m_sUserID;
			}
		...
		#region -- Log/Display 용 메시지 조합  ==> 미사용 ==> 향후 검토 기능
        ...
        public void LogToFile(string FileType, string FileName, string p_strStat, string p_strExplain, string p_strLogMsg)
        {
            string strLogMsg;
            string strLogFile;
            string LogPath = System.Configuration.ConfigurationSettings.AppSettings["LOG_PATH"].Trim();

            DateTime dt1 = DateTime.Now;
			// 20210414, SHS, 로그파일 경로 하위에 파일이름으로 폴더 지정
			LogPath = LogPath.EndsWith(@"\") ? $"{LogPath}{FileName}" : $@"{LogPath}\{FileName}";
			strLogFile = string.Format(LogPath + @"\{0}_{1}_{2}.TXT", FileType, FileName, dt1.ToString("yyyyMMdd"));
			//strLogFile = string.Format(LogPath + @"\{0}_{1}_{2}.TXT", "LOG", FileName, dt1.ToString("yyyyMMdd"));
            ...
            try
            {
                using (FileStream fs = new FileStream(strLogFile, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite))
                {
            ...
            DeleteOldFile("", FileName, LogPath, LogSaveDays, FileType);
        }

		//이후 함수 사용시 재설계후 정리 필요 ====================================

		#region -- 데몬 로그 파일 쓰기 == 미사용, 재정리 필요..
		...
		public void WriteStringDataToFile(string dataString, string filePath)
		{
			...
			try
			{
				...
				//파일명
				if(iCount > 3)  sb.Append(arrLogInfo[3]);
		...
		#region == 터미널 로깅[Multi Thread 환경](기존) ==  미사용
...
//            m_BefMsgStr = pMsg.MsgText;
		...
		#region == 터미널 로깅[Multi Thread 환경](Message Level 삭제) ==  미사용
        ...
        #region == 데몬 로깅(기존) ==  미사용 ==> 향후 필요시 정리 요망
		...
		#region == 데몬 로깅(Message Level 삭제) == 미사용 ==> 향후 필요시 정리 요망
		/// <summary>
		/// 데몬 프로그램 구동 시 로깅을 남김니다.
		/// </summary>
		/// <param name="msgID">메시지 아이디</param>
		...
		public string WriteLog(string msgID, clsProgramLogInfo programLogInfo, clsSourceInfo sourceInfo ,params string[] args)
		{
            ...
            return WriteLog("", msgID, programLogInfo, sourceInfo, args);
		}
        ...
        //==============================================
        #region -- LOG Delete == 로그 삭제관리 활용 가능
		...
		public class clsDelLogFile
		{
			...
			public clsDelLogFile(string sFilePath, string sProcessID, int nLogSaveDays, string fileType = "LOG")
			{
				m_FilePath		= sFilePath;
				m_ProcessID		= sProcessID;
				m_LogSaveDays	= nLogSaveDays;
				m_FileType		= fileType;
				if ( m_LogSaveDays<1 ) m_LogSaveDays = 1;
			}
			...
			public void DelProcLogFile()
			{
				...
				if(Directory.Exists(m_FilePath))
				{
					...
					//string[] dirs = Directory.GetFiles(m_FilePath, string.Format("{0}_*{1}*.TXT", "LOG", m_ProcessID));
					string[] dirs = Directory.GetFiles(m_FilePath, string.Format("{0}_*{1}*.TXT", m_FileType, m_ProcessID));
		...
		// 20210414, SHS, FILE TYPE 도 로그 지우는 메서드에 전달하여 처리
		//private void DeleteOldFile(string programID, string processID, string filePath, int logSaveDays)
		private void DeleteOldFile(string programID, string processID, string filePath, int logSaveDays, string fileType = "LOG")
		{
			...
			if(!m_htDelTime.Contains(strProcessID))
			{
				...
				clsDelLogFile pLogDel = new clsDelLogFile(sbFilePath.ToString(), strProcessID, dblLogSaveDays, fileType);
