...
namespace CFW.Common
{
	...
	public class Dictionary
	{
			...
			if(dicID != null)
			{
				...
				if(htDicList != null)
				{
					...
					if( dicInfo !=  null)
					{
						...
						{
							dicInfoNew.DicLabel = dicInfo.DicTextKo;
						}
						...
						if(dicInfoNew.DicLabel.Trim().Length == 0 )
						{
							if( dicInfo.DicText.Trim().Length ==0)
							{
								dicInfoNew.DicLabel =  dicID; //영어 용어도 없는 경우 ID 출력 
							}
							else
								dicInfoNew.DicLabel =  dicInfo.DicText; //해당 언어가 없는 경우 디폴트 영어로 출력	         
						}
					}
					else
					{
		...
		private static void ReadFromDB(DataSet	ds)
		{
			...
			{
				strMsgText = "DIC_NM_" +m_strDefaultLang.ToUpper().Replace("-","_");
			}
			else
			{
				strMsgText =  "DIC_NM_" +m_strWebDefaultLang.ToUpper().Replace("-","_");
			}
        ...
        public static void LoadStaticDicList()
        {
            ...
            try
            {
                ds = CFW.Data.OracleComDB.GetDicListWeb();
                if (((ds == null) || (ds.Tables.Count == 0)) || (ds.Tables[0].Rows.Count == 0))
                {
                    if (strLoadingType == "all")
                    {
                        ReadDicFromXml();
                    }
                }
