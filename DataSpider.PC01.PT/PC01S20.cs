...
namespace DataSpider.PC01.PT
{
    ...
    public class PC01S20 : PC00B01
    {
        ...
        public PC01S20(PC01F01 pOwner, DataRow dr, string equipType, string equipName, string connectionInfo, string extraInfo, int nCurNo, bool bAutoRun = false) : base(pOwner, dr, equipType, equipName, connectionInfo, extraInfo, nCurNo, bAutoRun)
        {
        }

        public PC01S20(PC01F01 pOwner, DataRow dr, int nCurNo, bool bAutoRun = false) : this(pOwner, dr, dr["EQUIP_TYPE_NM"].ToString(), dr["EQUIP_NM"].ToString(), dr["CONNECTION_INFO"].ToString(), dr["EXTRA_INFO"].ToString(), nCurNo, bAutoRun)
        {
            try
            {
        ...
        void InitOpcUaClient()
        {
            try
            {
                ...
                myUaClient = new OpcUaClient.OpcUaClient()
                {
                    ...
                    subjectName = Utils.Format($@"CN={m_Name}, DC={Dns.GetHostName()}")
