...
namespace DataSpider.PC01.PT
{
    ...
    /// Equip_Type : PCM_S470-K_V3 
    ///           : V2 버전기능 + 
    ///           : DC pH 중 BUFFER 가 2, 3, 4 세트만 오는 경우 (5 세트 기본) 처리
    /// </summary>
    public class PC01S33 : PC00B02
    {
        public PC01S33() : base()
        {
        }
        ...
        // 20240315, SHS, V3 용
        // V2 에 추가. DC pH 중 BUFFER 가 2, 3, 4 세트만 오는 경우 (5 세트 기본) 처리
        protected override void ParseMessage(string Msg)
        {
            ...
            if (listData.Count > 0)
            {
                ...
                // Buffer 1, 2, 3 와 Slope 1-2, Slope 2-3 까지 다 들어오는 상태로 태그 파싱이 되므로 이에 맞게 맞춰줌

                // Slope 2-3 이 안들어오는 경우 대비 처리

                // 20240315, SHS, V3
                // V2 부터는 PCM_S470-K 장비만 이 클래스를 사용하므로 타입확인은 삭제 
        ...
        // 20240315, SHS, V2 ParseMessage 백업
        //protected override void ParseMessage(string Msg)
        //{
        //    // 2022-07-25 kwc
        //    string[] LineData1 = Msg.Split(new string[] { System.Environment.NewLine }, StringSplitOptions.None);
